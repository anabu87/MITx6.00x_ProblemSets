...
class PhraseTrigger(Trigger):
    ...
    def evaluate(self, story):
        return (self._phrase in story.getTitle() or 
                self._phrase in story.getSubject() or
                 self._phrase in story.getSummary())
...
def filterStories(stories, triggerlist):
    ...
    for t in triggerlist:
        for story in stories:
            if t.evaluate(story) and (story not in fire_stories):
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == "TITLE":
        trigger = TitleTrigger(params_string)
        triggerMap[name] = trigger
    elif triggerType == "SUBJECT":
        trigger = SubjectTrigger(params_string)
        triggerMap[name] = trigger
    elif triggerType == "SUMMARY":
        trigger = SummaryTrigger(params_string)
        triggerMap[name] = trigger
    elif triggerType == "NOT":
        trigger = NotTrigger(triggerMap[params_string])
        triggerMap[name] = trigger
        ...
        trigger = PhraseTrigger(params_string)
        triggerMap[name] = trigger
    return trigger
