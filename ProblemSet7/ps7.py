...
class RectangularRoom(object):
    ...
    def getNumCleanedTiles(self):
        ...
        returns: an integer
        """
        num_of_clean_tiles = 0
        for v in self.tiles.values():
            if v == True:
                num_of_clean_tiles += 1
        return num_of_clean_tiles
    ...
    def getRandomPosition(self):
        ...
        """
        x = round(random.random() * self.width, 2)
        y = round(random.random() * self.height, 2)
        return Position(x, y)
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        # Test the robot hit the wall or not
        if self.room.isPositionInRoom(new_pos):
                self.pos = new_pos
                if not self.room.isTileCleaned(int(math.floor(new_pos.getX())),
                         int(math.floor(new_pos.getY()))):
                    self.room.cleanTileAtPosition(self.pos)
        else:
            self.angle = random.randint(0, 359)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        while min_coverage > float(cleaned_tiles) / float(total_tiles):
            for robot in robots:
